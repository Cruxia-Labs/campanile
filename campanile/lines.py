...
FLEET_NEGATIVE = ("Calibration: in a 55-repo fleet of living projects, "
                  ...
                  "specifically — the xz signature — 0 of 55.")
...
def render_scan_report(vec: dict, personas: bool = False) -> str:
    ...
    if not p:
        ...
        if personas:
            lines.append(f"  incumbent:  {p['incumbent']}")
            lines.append(f"  challenger: {p['challenger']}")
        else:
...
def render_agent_report(agent: dict) -> str:
    ...
    lines = ["", "  agent-held custody at HEAD:"]
    o = agent["overall"]
...
BASE_RATES_WIDE = (
    "Measured on a sealed 406-repo top-PyPI fleet, predictions staked "
    "before cloning: the bare xz shape in ANY file class — one class "
    "changes hands while the project overall never does — appears in 64 "
    "of 406 healthy repositories. With a 150-day recent-arrival condition "
    "on the ascending author, 36 of 406. Restricted to the tests class — "
    "the class that carried the xz payload — 17 of 406, about 1 in 24. "
    "At these rates the pattern alone accuses the innocent; that is the "
...
FAQ_HAND_LINE = (
    ...
    "keeps the whole. Reports speak in roles, and per-repo fleet results "
    "are never published: the only person who can show a repository's "
    "custody story is its maintainer. If this section was linked at you, "
    "the number being cited asks a question about succession; it does not "
    "answer one about intent.")
...
DISCLOSURE_PREAMBLE = (
    "The instrument measures structure. When a live repository shows the xz "
    "shape — a class-level custody inversion while overall custody never "
    "inverts, in a class listed in SIGNATURE_CLASSES_V0 — that observation "
    "is handled by this table, published in advance so the procedure is a "
    "commitment rather than a reaction. Coordinator-first: an established "
    "coordinating body receives the observation before anyone else, because "
    "a structural measurement about a living project deserves review by "
    "people equipped to weigh it.")

DISCLOSURE_ROW_ZERO = (
    "Row zero, absolute: any signature observation goes to founder review "
    "FIRST. No external contact of any kind — coordinator, maintainer, or "
    "public — precedes that review.")
...
DISCLOSURE_ROWS = (
    ("The xz shape at a living repository's current HEAD, reproducible "
     "at the pin",
     ...
     "publication"),
    ("The xz shape in an archived or unmaintained repository",
    ...
    ("A signature that does not reproduce at the current pin",
