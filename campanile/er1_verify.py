...
USAGE = ("usage: er1-verify [--pubkey KEY]... [--dsse] <receipt.json | golden_vectors.json> [...]\n"
         ...
         "  --dsse         inputs are DSSE envelopes (in-toto Statement, predicateType\n"
         "                 https://cruxia.dev/attestation/decision/v1): unwrap and verify the\n"
         "                 INNER receipt offline — the recomputation cosign cannot express.\n"
         "                 The envelope signature is reported (keyid) and verified only when\n"
         "                 --dsse-key PUBKEY_B64URL is given; absent that, the envelope\n"
         "                 layer is explicitly UNVERIFIED (said, never implied).")


def _dsse_unwrap(doc, dsse_key, path):
    """DSSE envelope -> the inner er1 receipt. Additive layer: nothing in the
    er1 verified claim changes — the inner receipt goes through the exact same
    pipeline. Structure errors are load failures (fail closed)."""
    import base64 as _b64
    if not isinstance(doc, dict) or "payload" not in doc or "payloadType" not in doc:
        raise Er1MalformedReceipt("not a DSSE envelope (payload/payloadType missing)")
    ptype = doc.get("payloadType")
    payload = _b64.b64decode(doc["payload"], validate=True)
    sigs = doc.get("signatures") or []
    keyid = sigs[0].get("keyid", "?") if sigs and isinstance(sigs[0], dict) else "?"
    env_note = f"envelope keyid={keyid} UNVERIFIED (no --dsse-key)"
    if dsse_key is not None:
        from cryptography.exceptions import InvalidSignature
        from cryptography.hazmat.primitives.asymmetric.ed25519 import Ed25519PublicKey
        t = ptype.encode("utf-8")
        pae = (b"DSSEv1 " + str(len(t)).encode() + b" " + t + b" "
               + str(len(payload)).encode() + b" " + payload)
        raw = _b64.urlsafe_b64decode(dsse_key + "=" * (-len(dsse_key) % 4))
        ok_env = False
        for s in sigs:
            try:
                Ed25519PublicKey.from_public_bytes(raw).verify(
                    _b64.b64decode(s.get("sig", ""), validate=True), pae)
                ok_env = True
                break
            except (InvalidSignature, ValueError, TypeError):
                continue
        if not ok_env:
            raise Er1MalformedReceipt("DSSE envelope signature INVALID for --dsse-key")
        env_note = f"envelope keyid={keyid} signature VERIFIED"
    statement = json.loads(payload.decode("utf-8"))
    if statement.get("predicateType") != "https://cruxia.dev/attestation/decision/v1":
        raise Er1MalformedReceipt(
            f"predicateType is {statement.get('predicateType')!r}, not the decision predicate")
    receipt = (statement.get("predicate") or {}).get("receipt")
    if not isinstance(receipt, dict):
        raise Er1MalformedReceipt("predicate carries no receipt object")
    subj = statement.get("subject") or [{}]
    print(f"DSSE  {path}  subject={subj[0].get('name', '?')} "
          f"sha256:{(subj[0].get('digest') or {}).get('sha256', '?')[:16]}…  {env_note}")
    return receipt
...
def main(argv=None) -> int:
    ...
    while i < len(argv):
        ...
        if a == "--dsse-key":
            if i + 1 >= len(argv):
                print("error: --dsse-key needs a value\n" + USAGE, file=sys.stderr)
                return 2
            dsse_key = argv[i + 1]
            i += 2
            continue
    ...
    try:
        for path in paths:
            try:
                ...
                if dsse:
                    doc = _dsse_unwrap(doc, dsse_key, path)
